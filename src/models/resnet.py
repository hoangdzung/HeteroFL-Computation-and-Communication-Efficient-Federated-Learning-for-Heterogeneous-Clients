...
class ResNet12(nn.Module):
    def __init__(self, data_shape, hidden_size, block, num_blocks, num_classes, rate, track):
        ...
        # self.layer4 = self._make_layer(block, hidden_size[3], num_blocks[3], stride=2, rate=rate, track=track)
        if cfg['norm'] == 'bn':
            n4 = nn.BatchNorm2d(hidden_size[2] * block.expansion, momentum=None, track_running_stats=track)
        elif cfg['norm'] == 'in':
            n4 = nn.GroupNorm(hidden_size[2] * block.expansion, hidden_size[3] * block.expansion)
        elif cfg['norm'] == 'ln':
            n4 = nn.GroupNorm(1, hidden_size[2] * block.expansion)
        elif cfg['norm'] == 'gn':
            n4 = nn.GroupNorm(4, hidden_size[2] * block.expansion)
        elif cfg['norm'] == 'none':
            n4 = nn.Identity()
        else:
            raise ValueError('Not valid norm')
...
def resnet12(model_rate=1, track=False):
    ...
    model = ResNet12(data_shape, hidden_size, Block, [1, 2, 2, 2], classes_size, scaler_rate, track)
